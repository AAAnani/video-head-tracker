...
from nha.models.identity_encoder import IdentityEncoder
from nha.models.pose_encoder import PoseEncoder

id_emb_dict = {'FLAME_SHAPE': 300,
'FLAME_TEX': 100,
'OFFSET_EMB': 512,
'TEXTURE_EMB': 512}
...
class FlameTracker:
    ...
    def _get_argparse_fields():
        ...
        fields = [
            # dataset
            dict(name_or_flags="--downscale_factor", default=1.0, type=float),
            # flags
    ...
    def __init__(self, **kwargs):
        ...
        self._flame = FlameHead(id_emb_dict['FLAME_SHAPE'], pose_emb_dict['FLAME_EXPR'])
        self._flame.to(self.device)


        self.identity_encoder = IdentityEncoder(embed_dict=id_emb_dict).to(self.device)
        self.pose_encoder = PoseEncoder(embed_dict=pose_emb_dict).to(self.device)

        self._flame_tex = FlameTex(id_emb_dict['FLAME_TEX'])
        self._flame_tex.to(self.device)

        if self._config["ignore_lower_neck"]:
            self._face_mask = torch.ones(len(self._flame.faces), device=self.device).bool()
            lower_neck_ids = np.load(FLAME_LOWER_NECK_FACES_PATH)
            self._face_mask[lower_neck_ids] = 0

    def predict_params(self, batch):
        batch_size = len(batch['rgb'])
        self.id_embedding, _ = self.identity_encoder(batch['identity_ims'].to(self.device), keys=['FLAME_SHAPE', 'FLAME_TEX']) # (batch_size, ims_per_id, channels, w, h)
        self._shape = self.id_embedding['FLAME_SHAPE']
        self._texture = self.id_embedding['FLAME_TEX']


        self.pose_embedding, _ = self.pose_encoder(batch['rgb'].to(self.device), 
                                    keys=['FLAME_EXPR', 'NECK_POSE', 'JAW_POSE', 'EYES_POSE', 'TRANSLATION', 'ROTATION', 'LIGHT', 'K', 'RT']) # (batch_size, channels, w, h)

        neutral = self._flame.get_neutral_joint_rotations()
        # offset all outputs with neutral values
        self.pose_embedding['NECK_POSE'] = neutral["neck"].detach().clone().repeat(batch_size, 1) + self.pose_embedding['NECK_POSE']
        self.pose_embedding['JAW_POSE'] = neutral["jaw"].detach().clone().repeat(batch_size, 1) + self.pose_embedding['JAW_POSE']
        self.pose_embedding['EYES_POSE'] = torch.cat([neutral["eyes"], neutral["eyes"]]).repeat(batch_size, 1) + self.pose_embedding['EYES_POSE']


        self._expr = self.pose_embedding['FLAME_EXPR']
        self._neck_pose = self.pose_embedding['NECK_POSE']
        self._jaw_pose = self.pose_embedding['JAW_POSE']
        self._eyes_pose = self.pose_embedding['EYES_POSE']
        self._translation = self.pose_embedding['TRANSLATION']
        self._rotation = self.pose_embedding['ROTATION']
        self._K = self.pose_embedding['K']
        self._RT = self.pose_embedding['RT'].reshape(-1, 3, 4)
        # light
        # initialize default light
        self._lights = torch.zeros(9, 3)
        self._lights[0] = (torch.tensor([np.sqrt(4 * np.pi)]).expand(3).float())
        self.pose_embedding['LIGHT'] = self._lights.repeat(batch_size, 1, 1).to(self.device) + self.pose_embedding['LIGHT'].reshape(-1, 9, 3)
        self._lights = self.pose_embedding['LIGHT'].reshape(-1, 9, 3)
        ...
        self._image_size = batch["rgb"].shape[-2:]
